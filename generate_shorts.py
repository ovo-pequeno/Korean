# =========================================================
# ひとくち韓国語｜Shorts（やさしい韓国語の短編ストーリー聞き流し）
# 【完全創作】Geminiが学習者向けのやさしい韓国語を作成。
...
def generate_story(avoid_summaries, max_retries=5):
    ...
    prompt = f"""You are a Korean teacher creating listening material for Japanese learners.
Write a very short, easy-to-listen original story in Korean (level: {LEVEL}).

Rules:
- Completely original. No real people, brands, or specific place names.
- Use simple, natural, everyday Korean suitable for learners.
...
{{
  ...
  "title_tgt": "short title in Korean",
  "sentences": [
    {{"tgt": "one sentence in Korean.", "ja": "その日本語訳"}}
  ]
}}
sentences: 6-9 items. Each 'tgt' is one natural Korean sentence with its 'ja' translation.{avoid_text}
"""
...
def build_video(data):
    ...
    title = data.get("youtube_title", "ひとくち韓国語")
    safe = title
    for ch in r'\/:*?"<>|':
...
def upload(youtube, path, title):
    description = (
        "やさしい韓国語の短いストーリーで、聞き流し韓国語リスニング。\n"
        "現地語と和訳つき。作業用・通勤用にどうぞ。\n\n#韓国語 #韓国語リスニング #韓国語学習 #聞き流し #shorts #Shorts"
    )
    body = {
        "snippet": {
            "title": (title + " #shorts")[:100],
            "description": description[:5000],
            "tags": ["韓国語", "韓国語リスニング", "韓国語学習", "聞き流し", "作業用BGM", "ハングル"],
            "categoryId": "27",
            "defaultLanguage": "ja",
        },
