# =========================================================
# ひとくち韓国語｜長尺（やさしい韓国語の短編ストーリー聞き流し・複数話）
...
Write a short, easy-to-listen original story in Korean (level: {LEVEL}).
...
def build_video(stories):
    ...
    main_title = stories[0].get("title_ja", "ひとくち韓国語")
    safe = main_title
    for ch in r'\/:*?"<>|':
    ...
    # エンディング
    end_txt = "오늘도 수고하셨습니다"
    a = make_audio_tgt("오늘도 수고하셨습니다", f"a_{idx}.mp3")
...
def upload(youtube, path, title, stories):
    ...
    description = (
        "やさしい韓国語の短いストーリーを集めた、聞き流し韓国語リスニング（現地語＋和訳つき）。\n"
        "作業用・睡眠用・通勤用にどうぞ。\n"
        f"Stories: {titles}\n\n#韓国語 #韓国語リスニング #韓国語学習 #聞き流し #作業用BGM"
    )
    body = {
        "snippet": {
            "title": title[:100],
